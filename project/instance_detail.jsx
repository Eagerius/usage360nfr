// Instance Detail View - opened when clicking an instance card from Client Detail
const {useState: useState3, useMemo: useMemo3, useEffect: useEffect3, useRef: useRef3} = React;

// ─── OBJECT DATA for each instance ───
...
function generateObjectsForInstance(instName) {
  ...
  // AT RISK indices: make ~4 objects at risk across the list
  ...
  return OBJECT_NAMES_ALL.map((name, idx) => {
    ...
    const threshold = Math.round(rng() * 8 + 18); // 18-26
    const isAtRisk = atRiskIndices.has(idx);

    const bars = [];
    for (let m = 0; m < 5; m++) {
      // At-risk objects: most bars exceed threshold
      const base = isAtRisk
        ? Math.round(rng() * 18 + threshold - 4) // mostly above threshold
        : Math.round(rng() * 20 + 5);             // mixed, generally lower
      bars.push(base);
    }
    // Last bar is always the prediction bar (lighter shade) — use similar range
...
// ─── Object Data Card with bar chart ───
function ObjectDataCard({ obj }) {
  const containerRef = useRef3(null);
  const chartRef = useRef3(null);
  const [dotHover, setDotHover] = useState3(false);
  const dotLeaveTimer = React.useRef(null);
  const deltaColor = obj.delta < 0 ? "var(--danger)" : "var(--teal-deep)";

  // Color each bar: above threshold = dark red, below = blue; last bar is lighter (prediction)
...
function InstanceDetailView({ inst, client, onBack, allClients }) {
  const [objTab, setObjTab] = useState3("All");
  const [searchQuery, setSearchQuery] = useState3("");
  const [simObj, setSimObj] = useState3(null);
  const [showSim, setShowSim] = useState3(false);

  const objects = useMemo3(() => generateObjectsForInstance(inst.name), [inst.name]);

  const filteredObjects = useMemo3(() => {
...
Object.assign(window, { InstanceDetailView, ObjectDataCard });
