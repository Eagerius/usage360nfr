// Simulate Plan Panel - right-side drawer for capacity simulation
const {useState: useState4, useEffect: useEffect4, useRef: useRef4, useMemo: useMemo4} = React;

function SimulatePlanPanel({ inst, obj, onClose }) {
  const [horizon, setHorizon] = useState4("6M");
  const [growthMode, setGrowthMode] = useState4("pct"); // "pct" | "abs"
  const [growthPct, setGrowthPct] = useState4(15);
  const [growthAbs, setGrowthAbs] = useState4(3000);
  const [teamExpansion, setTeamExpansion] = useState4(false);
  const [featureAdoption, setFeatureAdoption] = useState4(false);
  const [saved, setSaved] = useState4(false);
  const chartRef = useRef4(null);
  const containerRef = useRef4(null);

  // Derive base data from instance/object
  ...
  // Compute simulated end volume
  const appliedGrowth = useMemo4(() => {
  ...
  const thresholdSeries = Array.from({length: horizonMonths + 1}, () => threshold);
  ...
  const healthChange = Math.round(((simEndVolume - baseVolume) / threshold) * -30); // rough health delta
        ...
        <div style={{ flex: 1, padding: '0 24px 24px', display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 14, fontSize: 11, color: 'var(--ink-2)', fontFamily: 'Inter,sans-serif' }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                  <span style={{ width: 18, height: 2, background: '#B2B0B6', display: 'inline-block', borderRadius: 1, borderTop: '2px dotted #B2B0B6', background: 'none' }}></span>
                  Current
                </span>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
          ...
          <div style={{ padding: '16px 0 0', display: 'flex', gap: 10, alignItems: 'center', flexShrink: 0 }}>
            <button onClick={() => { setGrowthPct(15); setGrowthAbs(3000); setTeamExpansion(false); setFeatureAdoption(false); setGrowthMode("pct"); }}
              ...
              onMouseEnter={e => e.currentTarget.style.background = '#0B7E6C'}
              onMouseLeave={e => e.currentTarget.style.background = 'var(--teal-deep)'}
...
Object.assign(window, { SimulatePlanPanel });
